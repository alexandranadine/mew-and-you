...
function makeAnimal(overrides: Partial<RgAnimalResource> = {}): RgAnimalResource {
  return {
    type: 'animals',
    id: '12345',
    attributes: {
...
describe('mapRescueGroupsAnimal', () => {
  it('maps a complete animal record', () => {
    ...
    expect(cat.photos).toEqual([
      { url: 'https://example.org/photo-large.jpg', thumbnailUrl: 'https://example.org/photo-small.jpg' },
    ]);
    ...
    expect(cat.location).toEqual({ zip: '90026', city: 'Los Angeles', state: 'CA', lat: 34.09, lng: -118.27 });
  ...
  it('uses pictureThumbnailUrl as a fallback photo when no pictures are included', () => {
    ...
    expect(cat.photos).toEqual([{ url: 'https://example.org/thumb.jpg', thumbnailUrl: 'https://example.org/thumb.jpg' }]);
  });

  it('falls back to "Breed unknown" when no breed fields are present', () => {
    const animal = makeAnimal({ attributes: { breedPrimary: null, breedString: null } });
  ...
  it('uses breedString when breedPrimary is missing', () => {
    const animal = makeAnimal({ attributes: { breedPrimary: null, breedString: 'Tabby mix' } });
    const cat = mapRescueGroupsAnimal(animal, fullIncluded);
  ...
  it('maps an unrecognized or missing sex to "unknown"', () => {
    const missing = mapRescueGroupsAnimal(makeAnimal({ attributes: { sex: null } }), fullIncluded);
    const unrecognized = mapRescueGroupsAnimal(makeAnimal({ attributes: { sex: 'Unknown' } }), fullIncluded);
  ...
  it('falls back to zeroed coordinates and "Unknown" city when location and org data are both missing', () => {
    const animal = makeAnimal({ relationships: { orgs: undefined, locations: undefined } });
    const cat = mapRescueGroupsAnimal(animal, []);

    expect(cat.location).toEqual({ zip: '', city: 'Unknown', state: '', lat: 0, lng: 0 });
  });

  it('cleans up incomplete/messy descriptions and provides a fallback when blank', () => {
    const messy = mapRescueGroupsAnimal(
      makeAnimal({ attributes: { descriptionText: '  Sweet girl.&nbsp;&nbsp;Loves naps.  ' } }),
      fullIncluded,
    );
    const blank = mapRescueGroupsAnimal(makeAnimal({ attributes: { descriptionText: '' } }), fullIncluded);
    const nullDescription = mapRescueGroupsAnimal(makeAnimal({ attributes: { descriptionText: null } }), fullIncluded);
  ...
  it('falls back to an "Age unknown" label only when both ageString and ageGroup are missing', () => {
    const cat = mapRescueGroupsAnimal(makeAnimal({ attributes: { ageString: null, ageGroup: null } }), fullIncluded);
